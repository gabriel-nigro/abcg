...
void OpenGLWindow::initializeGL() {
  ...
  for (const auto index : iter::range(m_flyingAks)) {
    auto &position{m_flyingAksPositions.at(index)};
    auto &rotation{m_flyingAksRotations.at(index)};
...
void OpenGLWindow::randomizeFlyingAks(glm::vec3 &position,
                                      glm::vec3 &rotation) {
...
void OpenGLWindow::loadModel(std::string_view path) {
  ...
  m_model.loadNormalTexture(getAssetsPath() + "maps/ak_nmap.png");
