...
void Asteroids::update(const Square &ship, const GameData &gameData, float deltaTime) {
  for (auto &asteroid : m_asteroids) {
    asteroid.m_translation -= ship.m_velocity * deltaTime;
    asteroid.m_rotation = glm::wrapAngle(asteroid.m_rotation + asteroid.m_angularVelocity * deltaTime);
...
Asteroids::Asteroid Asteroids::createAsteroid(glm::vec2 translation,
  ...
  asteroid.m_velocity = glm::normalize(direction) / 4.0f;
