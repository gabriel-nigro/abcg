...
void StarLayers::paintGL() {
  ...
  for (const auto &layer : m_starLayers) {
    ...
    for (const auto j : {-2, 0, 2}) {
      abcg::glUniform2f(m_translationLoc, layer.m_translation.x + j, 0);
...
void StarLayers::update(const Square &square, float deltaTime) {
  for (auto &&[index, layer] : iter::enumerate(m_starLayers)) {
    const auto layerSpeedScale{1.0f / (index + 2.0f)};
    layer.m_translation -= square.m_velocity * deltaTime * layerSpeedScale;
